...
export default function Inventory() {
  ...
  const [inventoryData, setInventoryData] = useState<Medicine[]>(medicines);

  const categories = Array.from(new Set(inventoryData.map(medicine => medicine.category)));
  ...
  const handleAddMedicine = (medicine: Omit<Medicine, 'id'>) => {
    ...
    setInventoryData([...inventoryData, newMedicine]);
  };

  const handleUpdateMedicine = (id: string, updatedData: Partial<Medicine>) => {
  ...
  const handleDeleteMedicine = (id: string) => {
    setInventoryData(prevData => prevData.filter(medicine => medicine.id !== id));
  };
